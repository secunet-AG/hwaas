...
impl OperationOutput for ExtApiError {
    ...
    fn inferred_responses(
        ...
        let v = [
            (StatusCode::BAD_REQUEST, "Returned when invalid inputs were given in the request. \
                Fix: Check allowed range for inputs, the expected input types and ensure parses can be done correctly"),
            (StatusCode::UNAUTHORIZED, "Returned when the user did not provide any login credentials. Fix: Provide credentials for authentication."),
            (StatusCode::NOT_FOUND, "Returned when a switch received an incorrect ID, e.g. an Aruba Switch received a port ID of form accepted by Mellanox. \
                Fix: Check available ports at the switch that is to be configured."),
            (StatusCode::INTERNAL_SERVER_ERROR, "Returned when no correct request could be built to send to a switch. Fix: Check the IP provided or update API Version."),
            (StatusCode::BAD_GATEWAY, "Returned when a switch sent an unexpected response. \
                Fix: Check if switch is running and switching network is working as intended."),
            (StatusCode::SERVICE_UNAVAILABLE, "Returned when during connection setup a lock on a shared resource was not obtainable. Fix: Try again later."),
            (StatusCode::GATEWAY_TIMEOUT, "Returned when the requested switch could not be reached. \
...
impl From<SwitchApiError> for ExtApiError {
    fn from(e: SwitchApiError) -> Self {
        match e {
            ...
            SwitchApiError::Unauthorized => ExtApiError::new(StatusCode::UNAUTHORIZED, "No login credentials were provided. Required to access switches."),
            ...
            SwitchApiError::IDInvalid => ExtApiError::new(StatusCode::NOT_FOUND, "Please check your inputs and try again."),

            // Handle potential unexpected responses from the switch.
            SwitchApiError::UnexpectedResponseFromSwitch => ExtApiError::new(StatusCode::BAD_GATEWAY, "Ensure switch and switching network are available."),
