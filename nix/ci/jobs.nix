...
let
  ...
  belongsToGroup =
    group: target:
    target == group || lib.hasPrefix "${group}-" target;

  findGroup =
    target:
    lib.findFirst
      (group: belongsToGroup group target)
      null
      groups;
  ...
  isRegularCheck =
    ...
    lib.any
      (
        checkType:
        target == checkType || lib.hasSuffix "-${checkType}" target
      )
      checkTypes;
  ...
  mkJob =
    ...
    if items == [ ] then
      ...
      {
        inherit displayName group id needs phase;
  ...
  checksForGroup =
    group:
    builtins.filter
      (item: item.group == group && item.kind == "check")
      checks;

  testsForGroup =
    group:
    builtins.filter
      (item: item.group == group && item.kind == "test")
      checks;
  ...
  groupedJobs =
    lib.concatMap jobsForGroup groups;
  ...
  ungroupedChecks =
    builtins.filter
      (item: item.group == null && item.kind == "check")
      checks;

  ungroupedTests =
    builtins.filter
      (item: item.group == null && item.kind == "test")
      checks;
