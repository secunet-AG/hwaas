...
_: {
  perSystem = { config, pkgs, ... }: {
    packages = rec {
      net-ctrl-oas = pkgs.callPackage ./net-ctrl-oas.nix { inherit (config.packages) net-ctrl; };
      ...
      contextapi-oas = pkgs.callPackage ./contextapi-oas.nix { inherit (config.packages) contextapi; };
      hwaas-oas = pkgs.callPackage ./hwaas-oas.nix {
        inherit (config.packages) contextapi;
        inherit remote-hands-oas;
      };
      http-sim = pkgs.callPackage ./http-sim/package.nix { };
      vue-client = pkgs.callPackage ./vue-client.nix { vueSrc = ../../vue-client; };
