...
nixosTest {
  ...
  nodes = {
    ...
    # The websocketProxyClient is runing on this node and
    # connects to the counterpart runing on the server (aka. gateway).
  ...
  testScript = ''
    ...
    sut.wait_for_unit("iperf3-server.service")
    client.succeed("iperf3 -c ${sutIp} -p 7575 ")

    # Ping sut -> client
    sut.succeed("ping -c 1 ${clientIpTap}")
  '';
}
