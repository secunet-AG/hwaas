...
instance to follow along.

{/* If this is not the case, please refer to our [maintainer documentation](./managing-an-instance). */}
...
- A CI-based solution with a higher abstraction for scripting or interactive
  use: [User Tooling](#user-tooling)
- A low-effort interface for casual or debugging purposes: [Web UI](#web-ui)
...
building blocks used by other means of interaction like the [User Tooling](#user-tooling)
or [Web UI](#web-ui).
...
| Video          | Physical         | View a live V4L stream from the machine                    |
...
You can find usage examples of this in the [User Tooling Examples](#user-tooling-examples).
...
In contrast to the [Context API](../api) and [User Tooling](#user-tooling), the Web UI has
...
examples, replace `$API_URL` with the base URL of your HWaaS instance (e.g. `http://api.your-instance.com/v5`).
...
<Callout type="important">
    Store your Context ID carefully. Anyone with access to the ID and instance can control the
    machines in that context.
</Callout>
...
<Callout type="warning">
    Contexts have a configurable maximum total lifetime. You can increment your lifetime to keep it
    alive, but you cannot exceed this maximum amount without changing your deployment settings.
</Callout>
...
<Callout type="note">
    Images are cached by hash. If an identical image has already been uploaded, the existing hash is
    returned without re-uploading.
</Callout>

#### Attaching a Drive to Machines
...
<Callout type="warning">You must select an image for a machine before booting.</Callout>
...
You can also create the network in one request, moving the data into the command above.
...
You can read and write serial via `GET` and `POST` requests or directly connect
to the serial websocket with your favorite tooling via
`serial/<interface>/websocket`.
...
<Callout type="warning">
    Deleting a context releases all associated resources (drives, USB, networks, power). This cannot
    be undone.
</Callout>
...
<Callout type="note">
    The `hwaasTest` function handles context reservation, image uploading, and resource cleanup
    automatically. You only need to declare what machines and networks you need.
</Callout>
